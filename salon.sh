...
ENTER_PHONE(){
  ...
  echo -e "\nWhat is your phone number? (123-456-7890 or 12343452345)."
  sleep 1
  read CUSTOMER_PHONE
  CUSTOMER_PHONE=$(echo "$CUSTOMER_PHONE" | sed -E 's/(^[0-9]{3})([0-9]{3})([0-9]{4})$/\1-\2-\3/')
  echo $CUSTOMER_PHONE
  PHONE_MATCHES=$($PSQL "select phone from customers where phone='$CUSTOMER_PHONE'")
    if [[ ! $CUSTOMER_PHONE =~ ^[0-9]{3}-[0-9]{3}-[0-9]{4}$|^[0-9]{3}[0-9]{3}[0-9]{4}$ ]]
        then
        ...
        if [[ -z $PHONE_MATCHES ]]
          ...
          echo -e "\nThis number is already in our database.\nEnter another phone number."
          sleep 1
          ENTER_PHONE $SERVICE_ID_SELECTED
          fi
      fi
